...
def save(name, graph, model, overwrite=True):
    dirname = '%s/feynman2D_checkterms/%s/'%(model.workdir,name)
    try:
        os.mkdir('%s/feynman2D_checkterms'%model.workdir)
    except:
        pass
    try:
        os.mkdir(dirname)
...
def compile(name,model):
    calculate.compile("feynman2D_checkterms/%s/"%name, model)

def execute(name, model, points=10000, threads=4, calc_delta=0., neps=0):
    return calculate.execute("feynman2D_checkterms/%s/"%name, model, points=points, threads=threads, calc_delta=calc_delta, neps=neps)
