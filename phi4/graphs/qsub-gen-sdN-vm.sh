...
cd $WORKDIR
cat qsub-gen-sdN.tmpl|sed "s/<MODEL>/$MODEL/"|sed "s/<GRAPH>/$GRAPH/"| qsub  -N $NAME -l nodes=$NODESPPN -q long@pbs-vm.hpc.cc.spbu.ru -W group_list=vcluster -o logs/ -e logs/
