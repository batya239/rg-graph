...
def plot(coeffs, gStar, name, fileName):
    """
    ...
    plots = []
    for i in range(n):
        points = [sum(conformBorel(c,gStar,b=b_0+i)) for c in coeffs_by_loops]
        #print "i = ",i,points
        #xn, yn = array(map(float,L)),array(points)
        #print xn, yn
...
if __name__ == "__main__":
    ...
    plot(eta_g,gStar, '$\eta = \eta(L)$', 'pic_eta.pdf')
    #plot(beta_half,gStar, '$\\beta/2 = \\beta/2(L)$', 'pic_beta.pdf')
