...
def plot(coeffs, beta_half, name, b_0 = 2, d = 2):
    ...
    for i in range(4): ## loop over b: b=b_0+i
        ...
        points = [sum(conformBorel(c,gStar,loops = len(c)-1,b=b_local-3,n=1, dim = d)) for j,c in enumerate(coeffs_by_loops)]
        print "b = %.1f, g* = %f, \t"%(b_local,gStar),points
...
if __name__ == "__main__":
    ...
    plt1 = plot(eta_g,beta_half, '$\eta = \eta(n),\ d = %d,\ N = %d$'%(d,N), b_0 = b_0, d = d) ## <-- pic_eta_d3_n1.pdf
