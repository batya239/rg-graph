...
def plot(coeffs, beta_half, name, fileName):
    ...
    for i in range(4): ## loop over b: b=b_0+i
        ...
        try:
            ...
            plt.plot(x, fit_exp(x, *popt), '-', label="b = %.1f"%(b_0+i))
        except RuntimeError:
            pass
        # plots.append(plt.plot(L, points, 'o', label = 'b = %.1f'%(b_0+i)))
    title = name
...
def plotBeta(beta_half, name, b_0):
    ...
    try:
        ...
        lineType = 'o-'
    plots.append(plt.plot(L, points, lineType, label = '$g^* = g^*(L),\ b=%.1f$'%b_0))
...
if __name__ == "__main__":
    ...
    # plt1 = plotBeta(beta_half, '$g^* = g^*(L)$', b_0 = 3.0)
    # plt1 = plotBeta(beta_half, '$g^* = g^*(L)$', b_0 = 3.5)
    plt1 = plotBeta(beta_half, '$g^* = g^*(L)$', b_0 = 4.5)
    plt2 = plotBeta(beta_half, '$g^* = g^*(L)$', b_0 = 5.0)
    plt2 = plotBeta(beta_half, '$g^* = g^*(L)$', b_0 = 5.5)
    plt2.show()
