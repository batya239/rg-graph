...
def gen_speer_tree1(vars, cons, depth=None, symmetries=False, skip_bad_branches=False):
    tree = MSNTree(None, None, [])
    variables = vars
    if depth is None:
        raise NotImplementedError("depth=?")
    else:
        # add_speer_branches(tree, variables, cons, parents=list(), depth=depth, graph=graph if symmetries else None, skip_bad_branches=skip_bad_branches)
        add_speer_branches(tree, variables, cons, parents=list(), depth=depth, graph=None, skip_bad_branches=skip_bad_branches)
    return tree
...
def generate_func_files(tree, generate_expr_for_sector, eps_order=0):
    ...
    maxsectors = len([x for x in xMSNTreeElement2(tree)])
    count = 0
    for sector, coef in xMSNTreeElement2(tree):

        if count%100==0:
            print "sector %s of %s" %(count+1, maxsectors), map(lambda x: (x[0], len(x[1])), files.items())
        count+=1
        ...
        for term in expr:
            # print
            # print term
            extracted_for_term = pole_extractor.extract_poles_and_eps_series(term*coef, eps_order)
            # print extracted_for_term.keys()
            for key in extracted_for_term:
                if key in extracted:
                    extracted[key]+=extracted_for_term[key]
                else:
                    extracted[key]=extracted_for_term[key]
