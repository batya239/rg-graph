...
class PolynomialToolsTestCase(unittest.TestCase):
    def doTestDiff(self, expression, diffVar, vars, testPointsCount=20):
        ...
        sDiff = eval('diff(%s,%s)' % (formatter.format(expression, 'PYTHON'), multiindex.formatVar(diffVar)))
    ...
    def doTestSetNtoVar(self, expression, varToSet, n, vars, testPointsCount=20):
        ...
        myResExpr = eval(formatter.format(expression.set1toVar(varToSet) if n==1 else expression.set0toVar(varToSet), 'PYTHON'))
