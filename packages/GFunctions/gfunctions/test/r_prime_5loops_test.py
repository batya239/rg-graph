...
class RPrime5LoopsTestCase(gfunctions.test.GraphStorageAwareTestCase):
    ...
    def _compareRPrime(self, graphStateAsString, expected, useGraphCalculator=False):
        ...
        if len(g.edges(g.externalVertex)) == 2:
            ...
            withMomentumPassing = [x for x in momentum.xPassExternalMomentum(g, r_prime.defaultGraphHasNotIRDivergenceFilter)]
        calculated = False
        for graph in withMomentumPassing:
            ...
            if calculated:
                break
            try:
                actual = r_prime.doRPrime(graph, r_prime.MSKOperation(), r_prime.defaultSubgraphUVFilter,
