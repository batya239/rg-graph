...
def get_base_integrand_and_angles(graph_with_time_version, d_iw=False):
    v, scalar_products = time_versions.substitute(graph_with_time_version, d_iw=d_iw)
...
def construct_integrand_batch(integrands):
    dimension = configure_mr.Configure.dimension()

    main_integrand_a = symbolic_functions.CLN_ZERO
    integrand_e = None
    integrations = None
    scalar_products_functions = None

    loop_momentum_vars = integrands[0][0].loop_momentas
    stretch_vars = integrands[0][0].stretchers
    angles = integrands[0][0].scalar_products
    for i in integrands:
        assert loop_momentum_vars == i[0].loop_momentas
        assert stretch_vars == i[0].stretchers
        assert angles == i[0].scalar_products

    for (integral_repr, coeff) in integrands:
        base_integrand = integral_repr.integrand

        if configure_mr.Configure.debug():
            print "Integrand: %s\nLoop momentums: %s\nStretch vars: %s\nAngles: %s" % (base_integrand, loop_momentum_vars, stretch_vars, angles)

        integrand_e = symbolic_functions.CLN_ONE
        integrations = list()
    ...
    for s_v in stretch_vars:
        ...
        main_integrand_a = (symbolic_functions.CLN_ONE / swiginac.factorial(divergence)) * \
    ...
    #     print "D(a)Integrand(a):", main_integrand_a

    integrand_series = integrand_e.series(symbolic_functions.e == 0,
                                          configure_mr.Configure.target_loops_count() + 1 - len(
                                              loop_momentum_vars)).normal()
    ...
    for degree in xrange(integrand_series.ldegree(symbolic_functions.e), integrand_series.degree(symbolic_functions.e)):
        if degree == 0:
            ...
            integrand_series_map[degree] = integrand_e.subs(symbolic_functions.e == 0).normal() * main_integrand_a
        else:
            integrand_series_map[degree] = integrand_series.coeff(symbolic_functions.e ** degree).normal() * main_integrand_a
        if configure_mr.Configure.debug():
