...
def cross_sections_substitutions(graph, p2=False):
...
def construct_monomial(alpha_params):
    raw_result = dict()
...
def construct_feyn_repr_polynomials(graph):
    assert graph.edges()[0].alpha_param is not None
    ...
    determinant = set(map(lambda m: construct_monomial(m), determinant))
...
def get_polynomials(graph, p2=False):
    ...
    c, d = construct_feyn_repr_polynomials(graph)
    c_tau = tau_factor(graph)
    substitutor, v_params, v_power, tau_rate, c_omega = cross_sections_substitutions(graph, p2)
    if configure_mr.Configure.debug():
        print "C:", c
        print "D:", d
        print "C_tau:", c_tau
    ...
    return c, d, c_tau, c_omega, substitutor, v_params, v_power, tau_rate, graph
