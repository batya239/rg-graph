...
def generate_func_files(integrand_iterator, generate_function_body_=generate_function_body, max_function_length=maxFunctionLength):
    ...
    for eps_order, integrand_infos in integrand_iterator.iteritems():
        mappedIndexes = map(lambda i: len(i.getVarsIndexes()), integrand_infos)
        vars_count = max(mappedIndexes) if len(mappedIndexes) else 0
        if vars_count == 0:
            vars_count += 1
        file_info = funcFileInfo(eps_order, vars_count)
        function_file = files[file_info]
        for i in integrand_infos:
            function_file.add_function(generate_function_body_(polynomial.formatter.format(i, polynomial.formatter.CPP), list(i.getVarsIndexes()), "", "", ""))
...
def cuba_generate(series_dict):
    time_id = int(round(time.time() * 1000))
    directory = os.path.join("tmp/", str(time_id))
    directory = os.path.abspath(directory)

    generate_integrands(series_dict, directory, time_id)
    return directory
...
def cuba_integrate(series_dict):
    directory = cuba_generate(series_dict)
