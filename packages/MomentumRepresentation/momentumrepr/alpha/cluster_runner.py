...
OPERATION_NAMES[""] = lambda g: integration.integrands(g, "log")
OPERATION_NAMES["w"] = lambda g: integration.integrands(g, "iw")
OPERATION_NAMES["p"] = lambda g: None
...
from momentumrepr.cuba_integration import cuba_execute
from momentumrepr import configure_mr
import os
...
print dict(map(lambda (k, v): (k, (v.n * coefficient, v.s * coefficient)), d.iteritems()))
"""
...
def calculate_diagram(graph_state_str, operation_name, task_server_dir, aggregator_dir):
    ...
    for c, integrand in OPERATION_NAMES[operation_name](graph_state_str):
