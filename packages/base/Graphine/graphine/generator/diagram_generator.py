...
# -*- coding: utf8
#
# Generator of dynamical diagrams in statistical physics models. See example in "generate_percolation.py"
...
def generate_fields(graph_topology,
    ...
    assert graph_stub.external_edges_count == len(possible_external_fields), \
        "possible external field count (%s) must be same to graph leg count (%s)" % (len(possible_external_fields), graph_stub.external_edges_count)

    result = do_bfs(graph_stub, possible_fields_index, possible_external_fields, possible_vertices_rank, ignore_cycles, unlabeled_legs)
    return result


def do_bfs(graph_stub, possible_fields_index, possible_external_fields, possible_vertices_rank, ignore_cycles, unlabeled_legs):
    ...
    result = set()
    q = [(edges, vertices, possible_external_fields, None if ignore_cycles else Poset())]
    while len(q):
        edges, vertices, possible_external_fields, poset = q.pop()
        for obj in assign_fields_in_vertex(edges, vertices, possible_fields_index, possible_external_fields, possible_vertices_rank, poset, ignore_cycles):
            if isinstance(obj, tuple):
                q.append(obj)
            elif isinstance(obj, graphine.Graph):
                result.add(obj)

    if not unlabeled_legs:
        result = reduce(lambda x, y : x | label_legs(y), result, set())
    return result


def label_legs(graph):
    i_edges = graph.internal_edges

    external_edges_types = emptyListDict()
    for e in graph.external_edges:
        external_edges_types[e.fields[1]].append(e)

    result = {graphine.Graph(i_edges, renumbering=False)}
    for external_edges in external_edges_types.values():
        new_result = set()
        for g in result:
            edges_as_list = list(g.edges())
            for perm in itertools.permutations(external_edges):
                new_result.add(graphine.Graph(edges_as_list + map(lambda e: e[1].copy(fields=graph_state_config_with_fields.LabeledLegFields((e[0], e[1].fields[1]))), enumerate(perm))))
        result = new_result
    return result


def assign_fields_in_vertex(graph_edges, vertices, possible_fields_index, possible_external_fields, possible_vertices_rank, poset, ignore_cycles):
    vertex = vertices.pop()
    ...
    for i in possible_vertices_rank[len(vertex_edges)]:
        ...
        if supp is not None:
            ...
            for fields_perm in set(itertools.permutations(flatten_supp)):
                ...
                if suitable:
                    if len(vertices):
                        yield new_edges, list(vertices), new_possible_external_fields, None if ignore_cycles else new_poset
                    else:
