...
from rggraphenv import g_graph_calculator, StorageSettings, StoragesHolder, symbolic_functions
...
class RPrimeTestCase(unittest.TestCase):
    ...
    def _do_test_r1(self, graph_state_str, expected_result_as_string):
        g = graph_util2.graph_from_str(graph_state_str, do_init_weight=True)
        expected = symbolic_functions.evaluate(expected_result_as_string)
        actual = self.operator.kr_star(g, True).evaluate().simplify_indexed().expand()
