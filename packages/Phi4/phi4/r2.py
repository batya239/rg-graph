...
ValueAndPPower = collections.namedtuple("ValueAndPPower", ["value", "p_power"])
...
class RStar(object):
        ...
        for i in xrange(1, len(uv_subgraphs) + 1):
            for comb in itertools.combinations(uv_subgraphs, i):
                if i == 1 or not graphine.util.has_intersecting_by_vertices_graphs(comb):
                    shrunk, p2_counts = graph_util2.shrink_to_point(graph, comb)
                    ...
                    debug_line = "(%s)*KR*(%s)" % (symbolic_functions.CLN_MINUS_ONE ** i, comb) if self.debug() else None
    ...
    def kr_star_p2(self, graph, all_possible=False):
        ...
        if self.debug():
            print "KR*(%s)=%s" % (graph, result.evaluate())
        return result

    def kr_star(self, _graph, all_possible=False):
        uv_index = ir_uv.uv_index(_graph)
        if uv_index == 2:
            return self.kr_star_p2(_graph)
        evaluated = self.storage.get_graph((_graph.to_tadpole(), "kr_star"), "r")
        ...
        for graph in iterator:
            try:
                ...
                value = self.renormalize_ir(graph)
                ...
                if self.debug():
                    print debug_line
                    print "KR*(%s)=" % _graph + RStar.present_expression(current_result)
                if all_possible:
                    if result is None:
                        ...
                        print graph
                        assert result.evaluate().is_equal(current_result.evaluate())
                else:
                    self.storage.put_graph((_graph.to_tadpole(), "kr_star"), current_result, "r")
    ...
    def delta_ir(self, _graph):
        evaluated = self.storage.get_graph((_graph.to_tadpole(), "delta_ir"), "r")
        if evaluated is not None:
            return evaluated
        has_passings = False
        for graph in momentum.arbitrarily_pass_momentum(_graph):
            ...
            try:
                ...
                if self.debug():
                    print debug_line
                    print "D_IR(%s)=%s" % (_graph, RStar.present_expression(delta_ir))
                result = lazy.LazyValue(symbolic_functions.series(delta_ir.evaluate(), symbolic_functions.e, 0, 0, True))
                self.storage.put_graph((_graph.to_tadpole(), "delta_ir"), result, "r")
                return result
            except common.CannotBeCalculatedError:
                pass
        if not has_passings:
    ...
    def renormalize_ir(self, graph):
        evaluated = self.storage.get_graph((graph, "r_tilda"), "r")
        if evaluated is not None:
            return evaluated
        if RStar.is_tadpole(graph):
            ...
            renormalized_g = calculate_graph(graph)
            for co_ir in graph.x_relevant_sub_graphs(self._uv_filter + is_1uniting + no_hanging_parts,
                ...
                if ir_uv.uv_index(co_ir) > 0:
                    ...
                    raise common.CannotBeCalculatedError(graph)
                if self.debug():
                    debug_line += "+V(%s)*D_IR(%s)" % (co_ir, graph_util2.shrink_to_point(graph, (co_ir, ))[0])
                renormalized_g += calculate_graph(co_ir) * self.delta_ir(graph_util2.shrink_to_point(graph, (co_ir, ))[0])
            if self.debug():
                print debug_line
                print "R~(%s)=%s" % (graph, RStar.present_expression(renormalized_g))
            self.storage.put_graph((graph, "r_tilda"), renormalized_g, "r")
