...
def initialize():
    ...
    _MAIN_REDUCTION_HOLDER.set(ReductorHolder(two_loop_reductor, three_loop_reductor))
...
def _enumerate_graph(graph, init_propagators, to_sector=True):
    ...
    def _enumerate_next_vertex(remaining_propagators, _graph, vertex, result):
        if vertex not in graph_vertices:
            ...
            if len(e_.internal_nodes) == 2
            else None),
                            _graph.allEdges())
            result.add(graphine.Graph(new_edges, external_vertex))
        if vertex not in graph_vertices:
            return
            ...
            for propagator in (remaining_propagator, neg_propagator):
                if len(not_enumerated) == 1:
                    ...
                    if is_zero:
                        ...
                        new_graph = graphine.Graph(new_edges, externalVertex=external_vertex)
