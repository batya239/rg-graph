...
class TestFeynman(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for k in known.determinants.keys():
            self.feynman_reps[k] = feynman.Feynman(graphine.Graph.fromStr(k), theory=4)
...
class TestSectors(unittest.TestCase):
    ...
    def test_symmetries(self):
        for k in known.determinants.keys():
            g = graphine.Graph.fromStr(k)
            sym = feynman.sectors(g, symmetries=True)
            nsym = feynman.sectors(g, symmetries=False)
